# 在整个开发过程中进行了哪些测试，上线指标要求是什么？并发多少？
## 提到的优化设计，一定要说明：动机（面临了什么问题），设计了什么方案，得到了什么提升？是否有取舍？是否影响其它模块？
# 使用 API key 和 使用 vLLM 的区别在哪里？

# 项目架构核心

# 微服务架构：spring <-> AI-service
入口：前端用户消息给 spring，spring 发给 agent 服务，解析网关注入的 Header 带 token、用户信息
出口：基于 token 回调 spring 后端接口，或者直接调数据库（select只读）

## spring 后端、数据库

1. 表结构：Spring后端业务表 + AI表
业务表：workhour（工时填报记录，核心业务表）、project_info（项目信息）、sys_user（员工业务信息）、workhour_attendance（考勤、打卡、加班记录）、org_dept（部门组织结构）、work_calendar（工作日、节假日和应填工时日历）、project_member（项目与成员的关联关系）
AI 表：conversation_logs（AI 对话及工具调用审计）、ai_sessions（AI 会话汇总）

2. 调用的 spring 后端接口
- `GET /api/workhour/by-date-range`：工时查询、统计、周报、智能推荐、填报前校验；关联 `workhour`；请求 `startDate、endDate、memberId、projectId、orgId`，返回工时及项目、成员信息。
- `POST /api/workhour`：单条/批量工时填报；关联 `workhour`；请求 `memberId、projectId、workhourDate、workhour、workContent、workType、workhourType`，返回新建记录及 `id`。
- `POST /api/workhour/batch-approve`：工时审核；关联 `workhour`；请求工时记录 ID 数组，批量更新审核状态、审核人和审核时间。
- `GET /api/workhour/export/project-simple`：导出 Excel 工时报表；关联 `workhour、project_info、sys_user` 等；请求 `startDate、endDate、title、orgId`，返回包含人员、项目、日期和工时等汇总字段的 Excel 文件。
- `GET /api/project-infos`：项目列表、项目名称解析；关联 `project_info`；支持按项目名、ID、负责人等条件筛选及分页，返回项目、负责人、项目阶段和类型等信息。
- `GET /api/project-infos/{id}`：项目详情；关联 `project_info`；传入项目 `id`，返回项目、负责人、项目阶段和类型等信息。
- `GET /thsuaa/api/sys-users`：按姓名查用户 ID、用户查询；关联 `sys_user`；支持按名称、实体类型、组织或部门等条件筛选及分页，返回用户和组织信息。
- `GET /api/work-calendars/list`：判断工作日和工时类别；关联 `work_calendar`；支持按日期范围、删除状态筛选及分页，返回日期、是否工作日和删除状态。
- `POST /api/auth/mcp-token`：MCP 服务账号换取 JWT；关联 `jhi_user、sys_user` 等认证数据；请求 `entity_id、api_key`，返回 `token、userId、entityType` 等用户身份与权限信息。
...
一次 run 工具最大调用次数，调用同一工具多次，连续工具错误检测
...
| `compute_statistics` | 统计工时总量、占比等 | Spring Boot 数据＋Agent 计算 |
...
| `knowledge_qa` | 将制度、流程类问题路由到 RAG | RAG |
| `kb_outline` | 获取知识库目录 | 知识库 |
| `kb_keyword_search` | BM25 关键词检索 | 知识库 |
| `kb_semantic_search` | 向量语义检索 | 知识库 |
| `kb_read_section` | 读取指定文档章节 | 知识库 |

## 长短期记忆
基于用户身份、习惯、应用场景定制化的长短期记忆

## 保证稳定性、安全兜底、降级措施
先指出存在不稳定、不安全的环节，再枚举针对这些环境做了哪些措施
...
SQL Agent → mysql+aiomysql → workhour 数据库 → 执行 SELECT
允许直接查询七张表：workhour、project_info、sys_user、workhour_attendance、org_dept、work_calendar、project_member
适合 SpringBoot 固定接口难以完成的查询，例如：加班排名、跨表 JOIN、部门聚合、漏填工时反连接、项目成员分析
...
**针对 ..... 问题，设计了 ..... 解决方案，获取了 ... 收益，是否存在取舍和其它影响**
...
| --- | ---: | ---: | ---: |
| `qwen3.5-plus` | 83.3% | 约 46.7 s | 2.67 |
| `qwen3-8b` | 44.4% | 约 14.5 s | 1.00 |
| `qwen-flash` | 25.0% | 约 8.1 s | 4.33 |
